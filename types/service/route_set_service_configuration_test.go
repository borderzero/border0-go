...
func Test_ValidateRouteSetServiceConfiguration(t *testing.T) {
	t.Parallel()

	tests := []struct {
		name          string
		configuration *RouteSetServiceConfiguration
		expectError   bool
		expectedError string
	}{
		{
			name:          "Happy case for route set service with no cidrs",
			configuration: &RouteSetServiceConfiguration{},
			expectError:   false,
		},
		{
			name: "Happy case for route set service with only one v4 cidrs, no v6 routes",
			configuration: &RouteSetServiceConfiguration{
		...
		{
			name: "Happy case for route set service with only multiple v4 cidrs, no v6 routes",
			configuration: &RouteSetServiceConfiguration{
		...
		{
			name: "Happy case for route set service with only one v6 cidrs, no v4 routes",
			configuration: &RouteSetServiceConfiguration{
		...
		{
			name: "Happy case for route set service with only multiple v6 cidrs, no v4 routes",
			configuration: &RouteSetServiceConfiguration{
		...
		{
			name: "Happy case for route set service with multiple v4 and v6 cidrs",
			configuration: &RouteSetServiceConfiguration{
				IPv4CIDRRanges: []string{"66.66.66.66/24", "55.55.55.55/16"},
				IPv6CIDRRanges: []string{"2001:0db8:85a3::/64", "2001:0db8:85a4::/64"},
			},
